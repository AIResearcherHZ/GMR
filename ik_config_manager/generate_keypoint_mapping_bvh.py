...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["unitree_g1", "unitree_g1_with_hands", "unitree_h1", "unitree_h1_2",
                 ...
                 "openloong", "tienkung","joyin","joyin_add", "roboparty_atom01", "roboparty_atom02", "taks_t1", "semi_taks_t1", "semi_taks_lv1"],
        default="unitree_g1",
    ...
    src_frame_rot = ik_cfg_tmp.get("src_frame_rot", None)
    if src_frame_rot is not None:
        rot = R.from_quat([src_frame_rot[1], src_frame_rot[2], src_frame_rot[3], src_frame_rot[0]])
        root_pos = np.array(first_frame_data[retarget.human_root_name][0], dtype=np.float64)
        rotated = {}
        for name, (pos, quat) in first_frame_data.items():
            new_pos = rot.apply(np.array(pos, dtype=np.float64) - root_pos) + root_pos
            q_xyzw = [quat[1], quat[2], quat[3], quat[0]]
            new_q = (rot * R.from_quat(q_xyzw)).as_quat()
            rotated[name] = [new_pos, np.array([new_q[3], new_q[0], new_q[1], new_q[2]])]
        first_frame_data = rotated
    ...
    fk_solver = MuJoCoFK(_robot_xml)
    fk_solver.body_names = [mujoco.mj_id2name(fk_solver.model, mujoco.mjtObj.mjOBJ_BODY, i) for i in range(1, fk_solver.model.nbody)]
    joint_order = fk_solver.joint_order

    # 计算FK得到机器人T-pose
    qpos_fk = fk_solver.build_qpos(fixed_root_pos, fixed_root_rot, fixed_dof_pos)
    fixed_dof_pos = qpos_fk[7:] if fk_solver.has_freejoint else qpos_fk
