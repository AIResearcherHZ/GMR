...
def compute_quaternion_offsets(human_data, centers, Rs, body_names, ik_config):
    ...
    for table_key in ["ik_match_table1", "ik_match_table2"]:
        ...
        for robot_link, config in table.items():
            ...
            human_quat_xyzw = [human_quat[1], human_quat[2], human_quat[3], human_quat[0]]
            human_rot = R.from_quat(human_quat_xyzw)
            rot_offset = human_rot.inv() * robot_rot
            
            # 获取四元数偏移
            rot_offset_quat_xyzw = rot_offset.as_quat()
            rot_offset_quat = np.array([rot_offset_quat_xyzw[3], rot_offset_quat_xyzw[0], rot_offset_quat_xyzw[1], rot_offset_quat_xyzw[2]])
...
def compute_position_offsets(human_data, centers, Rs, body_names, ik_config, rot_offsets):
    ...
    for table_key in ["ik_match_table1", "ik_match_table2"]:
        ...
        for robot_link, config in table.items():
            ...
            human_quat_xyzw = [human_quat[1], human_quat[2], human_quat[3], human_quat[0]]
            human_rot = R.from_quat(human_quat_xyzw)
            rot_offset_xyzw = [rot_offset_quat[1], rot_offset_quat[2], rot_offset_quat[3], rot_offset_quat[0]]
