...
class MuJoCoFK:
    ...
    def __init__(self, asset_file: str, device=torch.device("cpu")):
        ...
        self.has_freejoint = (
            self.model.njnt > 0
            and int(self.model.jnt_type[0]) == int(mujoco.mjtJoint.mjJNT_FREE)
        )
        all_joints_xml = tree.getroot().find("worldbody").findall(".//joint")
        if self.has_freejoint:
            joints_for_axis = all_joints_xml[1:]
        else:
            joints_for_axis = all_joints_xml
        for j in joints_for_axis:
            axis_str = j.attrib.get("axis")
            if axis_str is None:
                continue
            self.dof_axis.append([float(i) for i in axis_str.split(" ")])
    ...
    def build_qpos(self, root_pos, root_rot, joints_dict):
        qpos = self.model.qpos0.copy().astype(np.float64)
        if self.has_freejoint:
            qpos[:3] = np.asarray(root_pos, dtype=np.float64).reshape(3)
            qpos[3:7] = np.asarray(root_rot, dtype=np.float64).reshape(4)
        assigned = 0
        for name, value in joints_dict.items():
            jid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, name)
            if jid < 0:
                continue
            qpos[int(self.model.jnt_qposadr[jid])] = float(value)
            assigned += 1
        print(f"[INFO] 映射关节: {assigned}/{len(joints_dict)}")
        return qpos
