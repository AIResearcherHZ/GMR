    ...
    class RenameUnpickler(pickle.Unpickler):
        def find_class(self, module, name):
            ...
            if module == "numpy._core":
                renamed_module = "numpy.core"
            elif module.startswith("numpy._core."):
                renamed_module = module.replace("numpy._core.", "numpy.core.")
...
def compute_local_body_pos(dof_pos, robot_type, device="cpu"):
    """
...
def convert_motion(src_data, src_robot, tgt_robot, device="cpu"):
    """
    ...
    if src_ankle_idx is not None and tgt_ankle_idx is not None:
        ...
        console.print(f"[cyan]  高度调整: {height_diff:.4f}m "
              f"(源踝关节z: {src_ankle_z:.4f}, 目标踝关节z: {tgt_ankle_z:.4f})[/cyan]")
    
    # Apply global z-axis offset
    if Z_OFFSET != 0.0:
        tgt_root_pos[:, 2] += Z_OFFSET
        console.print(f"[cyan]  应用全局Z偏移: {Z_OFFSET:.4f}m[/cyan]")
...
def main():
    console.print(Panel.fit("[bold cyan]机器人到机器人运动转换工具[/bold cyan]", border_style="cyan"))
    print_hardware_info()
    ...
    if args.src_robot == args.tgt_robot:
        console.print("[yellow]警告: 源机器人和目标机器人相同，无需转换。[/yellow]")
        return
    
    console.print(f"[bold cyan]从 {args.src_robot} 转换到 {args.tgt_robot}[/bold cyan]")
    console.print(f"  源文件: {args.src_file}")
    console.print(f"  目标文件: {args.save_path}")
    
    console.print("[cyan]加载源运动数据...[/cyan]")
    src_data = load_robot_motion(args.src_file)
    
    console.print(f"  帧数: {src_data['dof_pos'].shape[0]}")
    console.print(f"  FPS: {src_data['fps']}")
    console.print(f"  DOF: {src_data['dof_pos'].shape[1]}")

    console.print("[cyan]转换运动中...[/cyan]")
    tgt_data = convert_motion(src_data, args.src_robot, args.tgt_robot)
    
    console.print(f"  目标DOF: {tgt_data['dof_pos'].shape[1]}")
    
    console.print("[cyan]保存转换后的运动...[/cyan]")
    save_robot_motion(tgt_data, args.save_path)
    
    console.print(f"[bold green]完成! 已保存到 {args.save_path}[/bold green]")
